...
EXNIHILO is a permissionless token/USDC trading platform. Anyone can create a pool for any ERC-20 token. Pools support:

1. **Spot swaps** — constant-product AMM between token and USDC
2. **Leveraged longs** — buy token exposure without upfront collateral; pay only a 5% open fee
3. **Leveraged shorts** — sell synthetic token exposure; collateral locked in USDC
...
|---|---|---|
| Token / airToken | 18 | airToken always 18 dec regardless of underlying |
| USDC / airUsd | 6 | |
| LP NFT | — | ERC-721 |
| Position NFT | — | ERC-721 (ERC721Enumerable) |

`spotPrice()` formula assumes 18-decimal tokens.
...
|---|---|---|
| ACTOR-01 | Market Creator | Any wallet that calls the factory to deploy a new token market. Provides the initial token ratio (underlying token + USDC). Becomes the initial LP owner. |
| ACTOR-02 | LP Owner | Holder of the LP NFT for a given pool. Earns trading fees, can add more liquidity (same wallet only) |
| ACTOR-03 | Trader (Long) | Opens a leveraged long position on a token by paying USDC. Receives a Long Position NFT. |
| ACTOR-04 | Trader (Short) | Opens a leveraged short position on a token by paying USDC fees. Receives a Short Position NFT. |
...
|---|---|---|
| WRP-01 | airToken (airToken) | ERC-20 wrapper for the underlying token. Name pattern: `air[TokenSymbol]`. Minted 1:1 against deposited underlying tokens. Burns 1:1 on redemption. |
...
| FAC-02 | Initial ratio | The caller sets the initial token:USDC ratio by providing both amounts. The factory respects this ratio exactly; no normalisation is applied. |
| FAC-03 | Wrapper deployment | The factory deploys one airToken wrapper and one airUsd wrapper, both owned exclusively by the new pool contract. |
...
|---|---|---|---|
| SWAP-1 | Normal Swap | x = actual **backed** reserves of airToken; y = actual **backed** reserves of airUsd | Regular USDC ↔ Token trades. Internally wraps/unwraps tokens for the caller. |
| SWAP-2 | Long-open / Short-close Swap | x = actual **backed** reserves of airToken; y = **total supply** of airUsd (backed + synthetic) | Opening a LONG (airUsd → airToken) and closing a SHORT (airUsd → airToken). |
| SWAP-3 | Short-open / Long-close Swap | x = **total supply** of airToken (backed + synthetic); y = actual **backed** reserves of airUsd | Opening a SHORT (airToken → airUsd) and closing a LONG (airToken → airUsd). |
...
Returns raw USDC units (6 dec) per whole token (18 dec). Divide by 1e6 for a USD price. Only valid for 18-decimal tokens.
...
| DEX-03 | Auto wrap/unwrap | SWAP-1 accepts raw ERC-20 tokens (USDC or underlying token), wraps them internally before the swap, and unwraps the output before returning to the caller. |
...
| NFT-03 | Long NFT data | Stores: pool address, USDC amount used to open (`usdcIn`), airUsd minted amount (`airUsdMinted`), airToken amount locked (`airTokenLocked`), fees paid (USDC), block timestamp of open. |
| NFT-04 | Short NFT data | Stores: pool address, airToken minted amount at open rate (`airTokenMinted`), airUsd amount locked (`airUsdLocked`), fees paid (USDC), block timestamp of open. |
| NFT-05 | Token custody | The NFT contract holds custody of the locked wrapper tokens (airToken for longs, airUsd for shorts) for the duration of the position. |
...
### 4.1 Normal Swap (USDC ↔ Token)

| ID | Step | Requirement |
|---|---|---|
| NS-01 | Entry | Caller approves USDC (or underlying token) to the pool contract and calls `swap(amountIn, minAmountOut, direction)`. |
| NS-02 | Wrap | Pool wraps the input token into its respective wrapper (airUsd or airToken) using SWAP-1 backed reserves. |
...
*The user believes the token price will rise relative to USDC.*
...
| OL-03 | airUsd mint | The pool mints airUsd equal to the 100% `usdcAmount` (pre-fee, e.g. 10$) — the mint represents the user's collateral commitment.                                                                            |
| OL-04 | SWAP-2 execution | The 10 airUsd are swapped via **SWAP-2** (x = backed airToken, y = total airUsd supply) to receive airToken.                                                                                                 |
| OL-05 | NFT lock | The received airToken tokens are transferred to the Position NFT contract. A Long NFT is minted to the caller with fields: `usdcIn=10`, `airUsdMinted=10`, `airTokenLocked=<SWAP-2 output>`, `feesPaid=0.5`. |
...
| CL-02 | SWAP-3 execution | The locked airToken (from NFT data) is swapped via **SWAP-3** (x = total airToken supply, y = backed airUsd) to produce airUsd. |
...
| CL-07 | airToken not burned | The airToken wrappers are **not burned** at profitable close. The underlying token stays in the pool; burning wrappers without withdrawing underlying would permanently orphan those tokens. |

---

### 4.4 Open Short Position

*The user believes the token price will fall relative to USDC.*
...
| OS-03 | airToken mint | The pool calculates the current token/USDC rate using SWAP-1 backed reserves and mints the equivalent airToken for the specified USDC notional (e.g. 10 USDC notional → mint N airToken at current rate). |
| OS-04 | SWAP-3 execution | The minted airToken is swapped via **SWAP-3** (x = total airToken supply, y = backed airUsd) to produce airUsd. |
| OS-05 | NFT lock | The airUsd received is locked in the Position NFT contract. A Short NFT is minted to the caller with fields: `airTokenMinted=N`, `airUsdLocked=<SWAP-3 output>`, `feesPaid=<fees>`. |
| OS-06 | State update | Total airToken supply increases by N. Backed airToken reserves remain unchanged. |
...
| CS-02 | SWAP-2 execution | The locked airUsd (from NFT data) is swapped via **SWAP-2** (x = backed airToken, y = total airUsd supply) to produce airToken, but only the required amount of airToken, the surplus is kept as usd. |
| CS-03 | Profitability check | If returned airToken < `airTokenMinted` (stored in NFT), the transaction **REVERTS** with `"Position underwater"`.                                                                                 |
| CS-04 | airToken burn | The exact `airTokenMinted` amount of airToken is burned (synthetic debt repaid).                                                                                                                   |
...
*The holder takes physical delivery of the locked underlying tokens by repaying the synthetic USD debt. No AMM is involved; the exchange rate is the one locked in at open.*
...
| RL-04 | Token delivery | The `airTokenLocked` airToken held by the NFT contract is released, unwrapped 1:1 to the underlying token, and transferred to the NFT holder. |
...
*The holder takes physical delivery of the locked USDC by repaying the synthetic token debt. No AMM is involved; the exchange rate is the one locked in at open.*

| ID | Step | Requirement |
|---|---|---|
| RS-01 | Authorisation | Only the current ERC-721 holder of the Short NFT may call `realizeShort(nftId)`. |
| RS-02 | Input | Caller approves and pays exactly `airTokenMinted` underlying tokens (stored in the NFT) to the pool. |
| RS-03 | Debt repayment | The pool accepts the underlying tokens and burns `airTokenMinted` of synthetic airToken, clearing the position's debt. Backed airToken reserves increase by `airTokenMinted` (the incoming tokens now back the supply). |
...
| LIQ-02 | Collateral requirement | To force-close a **Long NFT**, the LP must pay `airUsdMinted` USDC. To force-close a **Short NFT**, the LP must pay `airTokenMinted` raw underlying tokens.    |
...
| SEC-04 | Reserve integrity | `backedAirToken ≤ airToken.totalSupply()` and `backedAirUsd ≤ airUsd.totalSupply()` after every operation. Violation reverts with `ReserveInvariantViolated()`. |
...
| EVT-05 | LongOpened | `pool: (nftId, holder, usdcIn, airUsdMinted, airTokenLocked, feesPaid)` |
| EVT-06 | LongClosed | `pool: (nftId, holder, profit, airUsdBurned)` |
| EVT-07 | ShortOpened | `pool: (nftId, holder, airTokenMinted, airUsdLocked, feesPaid)` |
| EVT-08 | ShortClosed | `pool: (nftId, holder, profit, airTokenBurned)` |
| EVT-09 | PositionForceRealized | `pool: (nftId, lpOwner, collateralPaid)` |
| EVT-10 | FeesClaimed | `pool: (lpOwner, amount)` |
| EVT-11 | LongRealized | `pool: (nftId, holder, usdcPaid, airUsdBurned, tokenDelivered)` |
| EVT-12 | ShortRealized | `pool: (nftId, holder, tokenPaid, airTokenBurned, usdcDelivered)` |
...
| UX-02 | Pool creation UI | Form to set token address, USDC amount, and token amount. Preview the calculated initial price ratio before confirming.                                                                                                                                         |
| UX-03 | Trade panel | Users select Long or Short, enter notional USDC amount, and preview: fee to pay, airUsd/airToken to be minted, estimated output from the relevant swap, and net exposure.                                                                                             |
| UX-04 | Position dashboard | Displays all open Long and Short NFTs held by the connected wallet. Shows current P&L (mark-to-market using current swap output vs. minted amount). Colour-coded profit/loss. Each position shows two actions: **Close** (AMM-based, profit in USDC) and **Realize** (direct delivery — underlying token for longs, USDC for shorts — by paying the original synthetic debt). |
...
Immutables (`airToken`, `airUsdToken`, `underlyingToken`, `underlyingUsdc`, `positionNFT`, `lpNftContract`, `lpNftId`, `protocolTreasury`, `swapFeeBps`) are inlined into bytecode at deployment.
...
|---|---|---|
| airToken | Wrapper token | ERC-20 wrapper for the underlying (non-USDC) asset in a pool. Named `air[TokenSymbol]`. |
...
| SWAP-2 | DEX mode | Long-open / short-close mode. Uses backed airToken as x, total airUsd supply as y. |
| SWAP-3 | DEX mode | Short-open / long-close mode. Uses total airToken supply as x, backed airUsd as y. |
