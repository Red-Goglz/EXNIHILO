...
const INITIAL_MEME = ethers.parseEther("1000000");  // 1,000,000 meme
...
async function deployFactoryFixture() {
  ...
  const factoryAddr = await factory.getAddress();
  await memeToken.mint(creator.address,  INITIAL_MEME * 3n);
  ...
  await memeToken.mint(creator2.address, INITIAL_MEME * 3n);
  await usdc.mint(creator2.address,      INITIAL_USDC * 3n);
  await memeToken.connect(creator2).approve(factoryAddr, ethers.MaxUint256);
  await usdc.connect(creator2).approve(factoryAddr, ethers.MaxUint256);

  return { factory, positionNFT, lpNft, memeToken, usdc, deployer, treasury, creator, creator2, other };
}

async function withOneMarketFixture() {
  const base = await deployFactoryFixture();
  const { factory, creator, memeToken } = base;
...
describe("EXNIHILOFactory", function () {
  ...
  describe("createMarket — happy path", function () {
    it("emits MarketCreated with correct fields", async function () {
      const { factory, creator, memeToken } =
    ...
    it("registers the pool in poolForToken", async function () {
      const { factory, poolAddress, memeToken } = await loadFixture(withOneMarketFixture);
      expect(await factory.poolForToken(await memeToken.getAddress())).to.equal(poolAddress);
    });
    ...
    it("pool's airMeme token is named correctly", async function () {
      const { poolAddress } = await loadFixture(withOneMarketFixture);
      const pool = await ethers.getContractAt("EXNIHILOPool", poolAddress);
      const airMeme = await ethers.getContractAt("AirToken", await pool.airMemeToken());
      expect(await airMeme.name()).to.equal("airPEPE");
      expect(await airMeme.symbol()).to.equal("airPEPE");
    });
  ...
  describe("createMarket — validation", function () {
    it("reverts when tokenAddress is the zero address", async function () {
      ...
      await expect(
        factory.connect(creator).createMarket(ethers.ZeroAddress, INITIAL_USDC, INITIAL_MEME, 0n, 0n)
      ).to.be.revertedWithCustomError(factory, "ZeroAddress");
    });

    it("reverts when usdcAmount is zero", async function () {
      const { factory, creator, memeToken } = await loadFixture(deployFactoryFixture);
      await expect(
        factory.connect(creator).createMarket(await memeToken.getAddress(), 0n, INITIAL_MEME, 0n, 0n)
    ...
    it("reverts when tokenAmount is zero", async function () {
      const { factory, creator, memeToken } = await loadFixture(deployFactoryFixture);
      await expect(
        factory.connect(creator).createMarket(await memeToken.getAddress(), INITIAL_USDC, 0n, 0n, 0n)
      ).to.be.revertedWithCustomError(factory, "ZeroAmount");
    });
    ...
    it("reverts when maxPositionBps is 5 (below minimum of 10)", async function () {
      const { factory, creator, memeToken } = await loadFixture(deployFactoryFixture);
      await expect(
        factory.connect(creator).createMarket(await memeToken.getAddress(), INITIAL_USDC, INITIAL_MEME, 0n, 5n)
    ...
    it("reverts when maxPositionBps is 9901 (above maximum of 9900)", async function () {
      const { factory, creator, memeToken } = await loadFixture(deployFactoryFixture);
      await expect(
        factory.connect(creator).createMarket(await memeToken.getAddress(), INITIAL_USDC, INITIAL_MEME, 0n, 9901n)
      ).to.be.revertedWithCustomError(factory, "InvalidMaxPositionBps");
    });
    ...
    it("accepts maxPositionBps of 10 (minimum boundary)", async function () {
      const { factory, creator, memeToken } = await loadFixture(deployFactoryFixture);
      await expect(
        factory.connect(creator).createMarket(await memeToken.getAddress(), INITIAL_USDC, INITIAL_MEME, 0n, 10n)
      ).to.emit(factory, "MarketCreated");
    });
    ...
    it("accepts maxPositionBps of 9900 (maximum boundary)", async function () {
      ...
      const meme2 = await MockF.deploy("DOGE", "DOGE", 18);
      await (meme2 as unknown as MockERC20).mint(creator.address, INITIAL_MEME);
      await (meme2 as any).connect(creator).approve(await factory.getAddress(), ethers.MaxUint256);
      await expect(
        factory.connect(creator).createMarket(await meme2.getAddress(), INITIAL_USDC, INITIAL_MEME, 0n, 9900n)
      ).to.emit(factory, "MarketCreated");
    });
    ...
    it("accepts maxPositionBps of 0 (disabled)", async function () {
      const { factory, creator, memeToken } = await loadFixture(deployFactoryFixture);
      await expect(
        factory.connect(creator).createMarket(await memeToken.getAddress(), INITIAL_USDC, INITIAL_MEME, 0n, 0n)
      ).to.emit(factory, "MarketCreated");
    });
  });
  ...
  describe("createMarket — multiple markets", function () {
    it("allPoolsLength increments for each market", async function () {
      const { factory, creator, creator2, memeToken, usdc } =
        await loadFixture(deployFactoryFixture);

      const MockF = await ethers.getContractFactory("MockERC20");
      const meme2 = await MockF.deploy("DOGE", "DOGE", 18) as unknown as MockERC20;
      await meme2.mint(creator2.address, INITIAL_MEME);
      await meme2.connect(creator2).approve(await factory.getAddress(), ethers.MaxUint256);

      await factory.connect(creator).createMarket(await memeToken.getAddress(), INITIAL_USDC, INITIAL_MEME, 0n, 0n);
      expect(await factory.allPoolsLength()).to.equal(1n);

      await factory.connect(creator2).createMarket(await meme2.getAddress(), INITIAL_USDC, INITIAL_MEME, 0n, 0n);
      expect(await factory.allPoolsLength()).to.equal(2n);
    });

    it("LP NFT IDs increment (first = 0, second = 1)", async function () {
      const { factory, creator, creator2, memeToken, usdc, lpNft } =
        await loadFixture(deployFactoryFixture);
      ...
      const MockF = await ethers.getContractFactory("MockERC20");
      const meme2 = await MockF.deploy("SHIB", "SHIB", 18) as unknown as MockERC20;
      await meme2.mint(creator2.address, INITIAL_MEME);
      await meme2.connect(creator2).approve(await factory.getAddress(), ethers.MaxUint256);
    ...
    it("second market for the same token does NOT overwrite poolForToken", async function () {
      const { factory, creator, creator2, memeToken } =
        await loadFixture(deployFactoryFixture);

      await factory.connect(creator).createMarket(await memeToken.getAddress(), INITIAL_USDC, INITIAL_MEME, 0n, 0n);
      const firstPool = await factory.poolForToken(await memeToken.getAddress());

      await factory.connect(creator2).createMarket(await memeToken.getAddress(), INITIAL_USDC, INITIAL_MEME, 0n, 0n);

      expect(await factory.poolForToken(await memeToken.getAddress())).to.equal(firstPool);
    });
  ...
  describe("_safeSymbol / _safeDecimals fallback", function () {
    it("falls back to 'TOKEN' when meme token has no symbol()", async function () {
      ...
      await (noMeta as any).mint(creator.address, INITIAL_MEME);
      ...
      expect(await airMeme.name()).to.equal("airTOKEN");
