...
2. The protocol takes a 5% base fee (4% to LP, 1% to protocol treasury) plus a dynamic impact fee (to LP) that scales with position size relative to pool liquidity
...
Every market caps a single position at a share of its USDC reserves — 1% when the
market opens, widening automatically to 20% over the first 24 hours. Exceeding it
reverts with `LeverageCapExceeded`.

Read `effectiveLeverageCap()` for the maximum notional in USDC right now. Nobody,
including the LP, can change it, and a position already open is never affected —
the check applies at open time only. See [Position Caps](/lp/position-caps).
